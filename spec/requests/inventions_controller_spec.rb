...
describe Api::V1::InventionsController do
  ...
  context 'it deletes an invention' do
    it 'found by id' do
      first_invention = Invention.first
      delete api_v1_invention_path(id: first_invention.id)
      expect(response.status).to be 204
      expect(Invention.where(id: first_invention.id).length).to be 0
    end

    it 'found by title' do
      delete destroy_by_title_api_v1_inventions_path(title: 'robot3')
      expect(response.status).to be 204
  ...
  context 'it updates invention' do
    ...
    it 'found by id' do
      patch api_v1_invention_path(id: @first_invention.id, invention: {title: 'robot44'})
      expect(response.status).to be 201
      expect(json_response['title']).to eq(Invention.find(@first_invention.id).title)
    end

    it 'found by title' do
      patch update_by_title_api_v1_inventions_path(title: @first_invention.title, invention: {title: 'robot44'})
      expect(response.status).to be 201
      expect(json_response['title']).to eq(Invention.find(@first_invention.id).title)
    end
  ...
  describe 'it creates a new invention' do
    it 'successfuly creates a new invention' do
      ...
      post api_v1_inventions_path(invention: new_invention)
      expect(response.status).to be 201
